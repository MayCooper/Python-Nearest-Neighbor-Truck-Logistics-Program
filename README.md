...
CSV Data Loading (get_csv_data function)<br>
Time Complexity: O(n) where n is the number of records in the CSV file. Each record is read sequentially.<br>
Space Complexity: O(n) as the data is stored in a list, consuming space proportional to the number of records.<br>
Loading Packages into Hash Map (load_packages_file function)<br>
Time Complexity: O(n) for reading and inserting n package records into the hash map. Although the average time complexity for each insert is O(1), reading n records results in a linear time complexity.<br>
Space Complexity: O(n) as each package object is stored in the hash map.<br>
Distance Calculations (distance_between function)<br>
Time Complexity: O(1) since the distance retrieval from a pre-loaded two-dimensional list is a direct access operation.<br>
Space Complexity: O(1) as no additional space is required for performing this operation.<br>
Address Index Retrieval (get_address function)<br>
Time Complexity: O(n) where n is the number of addresses. The function performs a linear search through the address list.<br>
Space Complexity: O(1) as the operation does not allocate additional space proportional to the input size.<br>
Package Delivery Simulation (deliver_packages function)<br>
Time Complexity: O(n^2) as it involves finding the nearest package for each undelivered package, which includes a nested operation of finding the minimum distance which itself iterates over the list of packages.<br>
Space Complexity: O(n) due to storing the list of not_delivered packages.<br>
...
Time Complexity:<br>
put: Average O(1), worst O(n) when resizing is needed.<br>
get: Average O(1), worst O(n) in case of many collisions.<br>
...
<h4>Optimized Data Access Patterns:</h4><br>
•	The software design strategically utilizes data structures such as lists and hash maps to optimize access patterns. For example, packages are accessed via their unique IDs using the hash map, ensuring constant-time complexity on average for queries. This leads to fast response times even as the number of packages grows.<br>
